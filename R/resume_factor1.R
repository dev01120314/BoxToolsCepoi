#' resume_factor1 gives a summary of indicator for factorial outcomes
#' @title Function \code{resume_factor1}
#' @rdname resume_factor1
#' @aliases resume_factor1
#' @export resume_factor1
...
#' @param vars_name a logical if TRUE the name of variables appear in the row name else if FALSE only modalities' names appear
...
resume_factor1 <- function(tab , idcol=seq( 1,dim(tab)[2],1), bookvarDispo=TRUE, bookvar, bookmod=NULL , vars_name=TRUE){
  # New table :
  nottreat <- tab[,-idcol]
  ...
  if (length(idcol ) == 1) {
    ...
    if (nlevels(temp) > 1) {
      for (j in seq_along( levels(temp) )) {
      ...
      if (nlevels(temp) > 1) {
        for (j in seq_along(levels(temp)) ) {
          tempX <- cbind( tempX, temp == levels(temp)[j] )
          ctempX <- c( ctempX, nameMode[j])
        }
      }else{
        tempX <- cbind( tempX, temp == levels(temp)[1] )
        ctempX <- c( ctempX,  nameMode[1])
      }
    }
